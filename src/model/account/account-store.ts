...
export class AccountStore {
	...
	readonly initialized = lazyObservablePromise(async () => {
		this.subscriptionPlans = observablePromise(loadPlanPricesUntilSuccess());
		...
		observe(this, "accountDataLastUpdated", () => {
			if (!this.user.subscription && this.user.teamSubscription) {
				alert(
	...
	@computed get userSubscription() {
		return this.user.userHasSubscription() ? this.user.subscription : undefined;
	}
